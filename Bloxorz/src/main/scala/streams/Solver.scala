...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    ...
    b.legalNeighbors map (n => (n._1, n._2 :: history))
    /*def neighboursWithHistoryInternal(neighbours: List[(Block, Move)]): LazyList[(Block, List[Move])] = {
      neighbours match {
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(LazyList[(Block, List[Move])]((startBlock, List[Move]())),Set[Block]())
